...
namespace DataFunc.Integrations.ExactOnline.Tests.GlClassifications
{
    public class GlClassificationsTestFixture : BaseTestFixture
    {
        ...
        {
            var expectedContentsToReturn = await ReadFileContents(Path.Combine("GlClassifications", "GlClassificationDetailResponse.json"));
            var service = GenerateService(CreateClientReturningSuccess(expectedContentsToReturn));

            var result = await service.GetDetail(0, Guid.Parse("7e0372ad-1fe2-42fb-8b69-414e1659279c"), CancellationToken.None);
            Assert.IsNotNull(result.Results);
        }
        ...
        {
            var expectedContentsToReturn = await ReadFileContents(Path.Combine("GlClassifications", "GlClassificationsListEmptyResponse.json"));
            var service = GenerateService(CreateClientReturningSuccess(expectedContentsToReturn));
